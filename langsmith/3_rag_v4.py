...
# --- CHANGE 1: Swapped imports from OpenAI to Google and Hugging Face ---
...
def build_vectorstore(splits, embed_model_name: str):
    # --- CHANGE 2: Swapped OpenAIEmbeddings for local HuggingFaceEmbeddings ---
    emb = HuggingFaceEmbeddings(model_name=embed_model_name)
    return FAISS.from_documents(splits, emb)

# ----------------- cache key / fingerprint (no change in logic) -----------------
...
def load_index_run(index_dir: Path, embed_model_name: str):
    # --- CHANGE 3: Swapped OpenAIEmbeddings for local HuggingFaceEmbeddings ---
...
def build_index_run(pdf_path: str, index_dir: Path, chunk_size: int, chunk_overlap: int, embed_model_name: str):
    print(f"No cache found. Building new index at {index_dir}...")
...
def load_or_build_index(
    ...
    # --- CHANGE 4: Updated default embedding model name ---
...
# --- CHANGE 5: Swapped ChatOpenAI for ChatGoogleGenerativeAI (Gemini) ---
...
# --- CHANGE 6: Added your project_name to the main decorator ---
@traceable(name="pdf_rag_full_run", project_name="RAG PDF App")
def setup_pipeline_and_query(
    pdf_path: str,
    question: str,
    chunk_size: int = 1000,
    chunk_overlap: int = 150,
    embed_model_name: str = "sentence-transformers/all-MiniLM-L6-v2", # <- Changed default
    force_rebuild: bool = False,
):
