...
# --- CHANGE 1: Added imports for Gemini, LangSmith Client, and Tracer ---
...
# ---------- Setup ----------
load_dotenv()
# --- CHANGE 2: Swapped ChatOpenAI for ChatGoogleGenerativeAI ---
model = ChatGoogleGenerativeAI(model="gemini-1.5-flash", temperature=0)

# ---------- Structured schema & model (no change in logic) ----------
class EvaluationSchema(BaseModel):
    feedback: str = Field(description="Detailed feedback for the essay")
    score: int = Field(description="Score out of 10", ge=0, le=10)

# Gemini models support structured output, so this works seamlessly
...
# ---------- Traced node functions (no change in logic) ----------
...
if __name__ == "__main__":
    # --- CHANGE 3: Set up the LangSmith Tracer for your project ---
    client = Client()
    tracer = LangChainTracer(project_name="langgraph-essay-checker-5", client=client)
    ...
    result = workflow.invoke({"essay": essay2}, config=config) #type:ignore
